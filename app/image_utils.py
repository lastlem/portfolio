    ...
    with app.app_context():
        ...
        try:
            with Image.open(orig_path) as img:
                ...
                img_1600.save(path_1600w, 'WEBP', quality=85, method=6, icc_profile=icc)

                # 800w for retina grid
                img_800 = img.copy()
                img_800.thumbnail((800, 800), Image.Resampling.LANCZOS)
                img_800.save(path_800w, 'WEBP', quality=85, method=6, icc_profile=icc)

                # 400w for standard grid
                img_400 = img.copy()
                img_400.thumbnail((400, 400), Image.Resampling.LANCZOS)
                img_400.save(path_400w, 'WEBP', quality=85, method=6, icc_profile=icc)
